...
?>
<?php if ($this->beginCache('pre-forum'. $forum->forum_name)): ?>
    <?php foreach($boards as $board): ?>
        <?php
            $totalRecords = count($board->subBoards);
            if (($board->parent_id !== PreForumBoard::AS_CATEGORY && $board->parent_id !== PreForumBoard::AS_BOARD)
                || ($board->parent_id === PreForumBoard::AS_CATEGORY && $totalRecords == 0)) {
                continue;
            }
        ?>
        <div class="tbox boarder">
            <?php if ($board->parent_id === PreForumBoard::AS_CATEGORY): ?>
                <div class="category">
                    <h2><?= Html::a(Html::encode($board->name), $board->url) ?></h2>
                </div>
            <?php elseif ($board->parent_id === PreForumBoard::AS_BOARD): ?>
                <div class="board" onclick="window.location.href= '<?= $board->url ?>';return false">
                    ...
                    <div class="hidden-xs pull-right" style="width:130px">
                        <i class="glyphicon glyphicon-user"></i> <?= PreForumBoard::getLastThread($board['id'])['username'] ?> <br>
                        <i class="glyphicon glyphicon-time"></i> <?= Yii::$app->formatter->asRelativeTime(PreForumBoard::getLastThread($board['id'])['created_at']) ?>
                    </div>
                ...
                <tbody>
                                        ...
                                        <td class="boardinfo" width="<?= (100.0 / $board->columns) ?>%" onclick="window.location.href= '<?= Url::toRoute(['/forum/board/view', 'id' => $subBoard['id']]) ?>';return false">
                                            ...
                                            <dl style="margin-bottom:0">
                                                ...
                                                <dd class="hidden-xs" data-toggle="tooltip" data-placement="top" title="Thread Count">
                                                    <i class="glyphicon glyphicon-comment"></i> <?= PreForumBoard::getThreadCount($subBoard['id']) ?> 
                                                </dd>
                                                <dd class="hidden-xs"><i class="glyphicon glyphicon-time"></i> <?= Yii::$app->formatter->asRelativeTime(PreForumBoard::getLastThread($subBoard['id'])['created_at']) ?></dd>
                                            </dl>
                                        </td>
                                    <?php endif ?>
                                    <?php $counter += 1; ?>
                    ...
                    <?php else: ?>
                        <?php foreach ($board->subBoards as $subBoard): 
                            $lastThread = PreForumBoard::getLastThread($subBoard['id']);
                        ?>
