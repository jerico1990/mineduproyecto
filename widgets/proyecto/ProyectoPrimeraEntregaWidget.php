...
class ProyectoPrimeraEntregaWidget extends Widget
{
    ...
    public function run()
    {
        ...
        $proyecto=ProyectoCopia::find()->where('equipo_id=:equipo_id',[':equipo_id'=>$integrante->equipo_id])->one();
        $objetivos_especificos=ObjetivoEspecificoCopia::find()->where('proyecto_id=:proyecto_id',[':proyecto_id'=>$proyecto->id])->all();
        $actividades=ActividadCopia::find()
                    ...
                    ->where('proyecto_id=:proyecto_id and actividad_copia.estado=1',[':proyecto_id'=>$proyecto->id])->all();
        $i=1;
